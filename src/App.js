...
class App extends Component {
  ...
  componentDidMount = () => {
    ...
    const timerId = setInterval(() => {
      ...
      if (seconds === 1) {
        ...
        console.log('katham')
        this.setState({seconds: 0, progress: 'over'})
      }
      this.setState({
        seconds: seconds - 1,
        timerId,
  ...
  filterImgList = () => {
    ...
    const filtered = imagesList.filter(each => each.category === activeBtn)
    return filtered
  }
  ...
  randomImgFunc = () => {
    ...
    const imgObj = imagesList[imgIndex]
    console.log(imgIndex, imgObj)
